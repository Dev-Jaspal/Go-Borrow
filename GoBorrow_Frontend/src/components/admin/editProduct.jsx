import React from 'react';
import './editProduct.css';
import { useNavigate, useParams } from 'react-router-dom';
import allProducts from '../../services/allProducts'
import { useState } from 'react';

function EditProduct() {
    const {userName} = useParams();
    const [products, setProducts] = useState(allProducts);

    const navigate = useNavigate();

    // Base64 string data
    const data = 'data:image/jpeg;base64,'
    
    const handleDelete = (id) =>{
        if (window.confirm("Do you want to delete this item?")){
            const updatedProduct = products.filter((user) => (user.id !== id))
            // setProducts(updatedProduct);
            // <allProducts setProducts={updatedProduct} />
            // console.log(updatedProduct);
        }
        navigate('/admin');
    }

    const handleSave = (event) =>{
        event.preventDefault();

        navigate('/admin');
    }

    return ( <>
        {products.filter((user, idx) => user.username === userName).map((product, idx) => 
        
        <main className="d-flex align-items-center min-vh-100 py-3 py-md-0">
            <div className="container">
            <div className="prodDetail-card">
                <div className="row no-gutters">
                <div className="col-md-5">
                    <img src={`${data}`} alt="prodDetail" className="prodDetail-card-img"/>
                </div>
                <div className="col-md-7">
                    <div className="card-body">
                    <p className="prodDetail-card-description">Product Detail</p>
                    <form>
                    <div className="form-group">
                        <label htmlFor="name" >Product Name: <input name="p-item" defaultValue={product.item}/></label>
                    </div>
                    <div className="form-group">
                        <label htmlFor="name" >Product Price: <input name="p-price" defaultValue={product.price} /></label>
                    </div>
                    <div className="form-group">
                        <label htmlFor="name" >Product Condition: <input name="p-condition" defaultValue={product.condition} /></label>
                    </div>
                    <div className="form-group">
                        <label htmlFor="name" >Product Category: <input name="p-category" defaultValue={product.category} /></label>
                    </div>
                    <div className="form-group">
                        <label htmlFor="name" >Product Bought Year: <input name="p-date" defaultValue={product.date} /></label>
                    </div>
                    <div className="form-group">
                        <label htmlFor="name" >Product Location: <input name="p-location" defaultValue={product.location} /></label>
                    </div>
                    <div className="form-group">
                        <label htmlFor="name" >Product Description: <input name="p-description" defaultValue={product.description} /></label>
                    </div>
                    <input name="register" id="register" className="btn btn-inline-block btn-secondary mb-4" type="button" value="Delete"
                        onClick={()=>handleDelete(product.id)}/>
                    <input name="register" id="register" className="btn btn-inline-block btn-success mb-4 ml-2" type="button" value="Save"
                        onClick={handleSave}/>
                    
                    </form>
                    </div>
                </div>
                </div>
            </div>
            </div>
        </main>
        )}
    </>);
}
 
export default EditProduct;